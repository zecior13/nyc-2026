...
function renderQueensGuide(day, guide) {
  return `<nav class="day-tabs" aria-label="Sekcje dnia"><button class="day-tab active" data-day-panel="overview">Plan</button><button class="day-tab" data-day-panel="route">Trasa</button><button class="day-tab" data-day-panel="food">Jedzenie</button><button class="day-tab" data-day-panel="tennis">US Open</button><button class="day-tab" data-day-panel="basics">Tenis 101</button><button class="day-tab" data-day-panel="variants">Warianty</button><button class="day-tab" data-day-panel="links">Mapy</button></nav>
  <section class="day-panel active" data-panel="overview"><div class="sheet-section"><h3>Plan dnia · wybierz punkt</h3>${timeline(day.items,guide.timelineTargets)}</div><div class="quick-filters"><button data-related-panel="route">Miejsca i historie</button><button data-related-panel="food">Smaki</button><button data-related-panel="basics">Dla laików</button><button data-related-panel="tennis">Stadion</button></div><div class="sheet-section"><h3>Najważniejsze</h3><div class="simple-card"><p>${day.essentials.join("<br>")}</p></div></div></section>
  <section class="day-panel" data-panel="route" hidden><div class="section-heading-row"><h3>Queens · od portu do kortów</h3><span>sprawdzono ${guide.checked}</span></div><p class="panel-intro">Numery 1–5 są identyczne na mapie i w opisie. Historie rozwijają się przy miejscach, których dotyczą.</p><div class="route-list">${guide.route.map((x,i)=>{const numbers=["1","2","→","3","4","5"];return `<article class="route-stop" data-content-key="q-stop-${i}"><div class="route-number ${i===2?"route-number-transit":""}">${numbers[i]}</div><div><span class="mini-kicker">${x.time}</span><h4>${x.title}</h4><p>${x.text}</p><div class="look-box"><strong>Rozejrzyj się:</strong> ${x.look}</div>${inlinePlaceStories(x.related,guide.stories)}${inlineRelatedActions(x.related)}</div></article>`}).join("")}</div></section>
  ${renderFoodPanel(guide)}
