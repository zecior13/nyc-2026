...
function renderSohoYankeesGuide(day, guide) {
  return `<nav class="day-tabs" aria-label="Sekcje dnia"><button class="day-tab active" data-day-panel="overview">Plan</button><button class="day-tab" data-day-panel="route">SoHo</button><button class="day-tab" data-day-panel="shopping">Zakupy</button><button class="day-tab" data-day-panel="food">Jedzenie</button><button class="day-tab" data-day-panel="pier35">Pier 35</button><button class="day-tab" data-day-panel="rest">Reset</button><button class="day-tab" data-day-panel="transport">Dojazd</button><button class="day-tab" data-day-panel="stadium">Stadion</button><button class="day-tab" data-day-panel="baseball">Baseball 101</button><button class="day-tab" data-day-panel="variants">Warianty</button><button class="day-tab" data-day-panel="links">Mapy</button></nav>
  <section class="day-panel active" data-panel="overview"><div class="sheet-section"><h3>Plan dnia · wybierz punkt</h3>${timeline(day.items,guide.timelineTargets)}</div><div class="quick-filters"><button data-related-panel="shopping">Dla Matyldy</button><button data-related-panel="route">Miejsca i historie</button><button data-related-panel="pier35">Huśtawki i zdjęcia</button><button data-related-panel="baseball">Dla laików</button><button data-related-panel="stadium">Mecz</button></div><div class="sheet-section"><h3>Najważniejsze</h3><div class="simple-card"><p>${day.essentials.join("<br>")}</p></div></div></section>
  <section class="day-panel" data-panel="route" hidden><div class="section-heading-row"><h3>SoHo i Nolita · fasady, nie lista sklepów</h3><span>sprawdzono ${guide.checked}</span></div><p class="panel-intro">Punkty 1a–1d rozwijają pierwszy punkt mapy dnia. Zakupy, lunch i Pier 35 mają własne numery 2–4.</p><div class="route-list">${guide.route.map((x,i)=>`<article class="route-stop"><div class="route-number">1${String.fromCharCode(97+i)}</div><div><span class="mini-kicker">${x.time}</span><h4>${x.title}</h4><p>${x.text}</p><div class="look-box"><strong>Rozejrzyj się:</strong> ${x.look}</div>${inlinePlaceStories(x.related,guide.stories)}${i===guide.route.length-1?`<details class="place-story"><summary>${guide.stories[3].title}</summary><p>${guide.stories[3].text}</p></details>`:""}</div></article>`).join("")}</div></section>
  <section class="day-panel" data-panel="shopping" hidden><h3>Zakupy Matyldy · wybór, nie maraton</h3><div class="notice">Wybierzcie dwa sklepy główne i maksymalnie jeden szybki. O 13:00 kończą się zakupy niezależnie od kolejek — inaczej wypada Pier 35.</div>${matyldaShoppingWishlist("soho")}</section>
  ${renderFoodPanel(guide)}
  <section class="day-panel" data-panel="pier35" hidden><span class="event-status">${guide.pier35.status}</span><h3>${guide.pier35.title}</h3><p class="panel-intro">${guide.pier35.address}<br>${guide.pier35.intro}</p><div class="event-grid"><article class="event-card"><h4>Po kolei</h4><ul>${guide.pier35.route.map(x=>`<li>${x}</li>`).join("")}</ul></article><article class="event-card"><h4>Zdjęcia</h4><ul>${guide.pier35.photo.map(x=>`<li>${x}</li>`).join("")}</ul></article></div><details class="place-story"><summary>${guide.stories[5].title}</summary><p>${guide.stories[5].text}</p></details><div class="notice">${guide.pier35.fallback}</div></section>
...
function renderDepartureGuide(day, guide) {
  ...
  <section class="day-panel" data-panel="walk" hidden><div class="section-heading-row"><h3>42nd Street · ostatnie spojrzenie</h3><span>sprawdzono ${guide.checked}</span></div><div class="route-list">${guide.walk.map((x,i)=>`<article class="route-stop" data-content-key="departure-walk-${i}"><div class="route-number">${i+1}</div><div><span class="mini-kicker">${x.time}</span><h4>${x.title}</h4><p>${x.text}</p><div class="look-box"><strong>Rozejrzyj się:</strong> ${x.look}</div>${inlinePlaceStories(x.related,guide.stories)}${x.title.includes("Grand Central")?`<details class="place-story"><summary>${guide.stories[4].title}</summary><p>${guide.stories[4].text}</p></details>`:""}</div></article>`).join("")}</div></section>
...
const MODERN_DAY_MODULES = {
  ...
  sohoyankees:[['overview','01','Plan','SoHo, Pier 35 i baseball'],['route','◇','Miejsca SoHo','fasady i Nolita'],['shopping','☆','Zakupy','moduł Matyldy'],['food','☕','Jedzenie','szybki lunch i stadion'],['pier35','≈','Pier 35','huśtawki i East River'],['rest','H','Reset','powrót do hotelu'],['transport','→','Dojazd','metro do Bronksu'],['stadium','⚾','Yankee Stadium','rytuał meczu'],['baseball','?','Baseball 101','zasady dla laików'],['links','↗','Mapy','trasy i powrót']],
...
const TRIP_REGIONS = [
  { id: "midtown", name: "Midtown", color: "#f5c518", text: "Wasza baza i najbardziej intensywny obraz Manhattanu: Times Square, M Social, Broadway, Bryant Park, Rockefeller Center i Grand Central.", days: ["2026-08-22", "2026-08-23", "2026-08-24", "2026-08-25", "2026-08-26", "2026-08-30"] },
  ...
  { id: "downtown", name: "Lower Manhattan i port", color: "#d37845", text: "Najstarszy Nowy Jork, finanse, pamięć o 11 września, Pier 35, heliport oraz widoki na port i Statuę Wolności.", days: ["2026-08-27", "2026-08-28", "2026-08-29"] },
...
const PLACE_EXTRAS = {
  ...
  "pier-35":{region:"downtown",status:"w planie · 28.08",why:"Huśtawki i East River dają pół godziny odpoczynku po zakupach bez tworzenia osobnej wyprawy.",curiosity:"Park otwarto w 2019 roku na dawnym przemysłowym nabrzeżu; łączy duże huśtawki, zieloną ścianę i siedlisko pływowe Mussel Beach.",photoTip:"Fotografujcie huśtawki lekko z boku, pozostawiając Manhattan Bridge i wodę w tle.",nearby:["soho","downtown"]},
...
const DAY_SCHEMATICS = {
  ...
  "2026-08-28": { note: "SoHo schodzi przez Chinatown na East River; po krótkim resecie na Pier 35 wracacie do hotelu i jedziecie do Bronksu.", nodes: [["Hotel", "midtown", "⌂", "overview"], ["SoHo", "village", "◆", "route"], ["Pier 35", "downtown", "≈", "pier35"], ["Hotel", "midtown", "⌂", "rest"], ["Yankee Stadium", "bronx", "⚾", "stadium"]], legs: ["metro ok. 20 min", "spacer przez Nolita/Chinatown", "metro F lub Uber", "metro ok. 35 min"] },
...
const DAY_VISUALS = {
  ...
  "2026-08-28": { area:"SoHo → East River → Bronx", icon:"⚾", event:"Pier 35 · Yankees–Red Sox", color:"#365c7f" },
...
const DAY_MAP_CONFIGS = {
  ...
  "2026-08-28": { title:"SoHo → Pier 35 → hotel → Yankee Stadium", view:[0,150,862,1000], points:[
    {panel:"overview",title:"Hotel · start",note:"Wcześniejszy wyjazd do SoHo",x:430,y:790,icon:"H"},{panel:"route",title:"SoHo",note:"Punkt 1 · żeliwne fasady",x:450,y:1040,icon:"1"},{panel:"shopping",title:"Zakupy",note:"Punkt 2 · koniec bezwzględnie o 13:00",x:465,y:1020,icon:"2"},{panel:"food",title:"Lunch",note:"Punkt 3 · Chinatown lub Lower East Side",x:490,y:1080,icon:"3"},{panel:"pier35",title:"Pier 35",note:"Punkt 4 · huśtawki i East River",x:555,y:1115,icon:"4"},{panel:"rest",title:"Hotel · reset",note:"Powrót około 15:30 i odpoczynek przed meczem",x:445,y:800,icon:"H"},{panel:"stadium",title:"Yankee Stadium",note:"Punkt 5 · Yankees–Red Sox w Bronksie",x:700,y:275,icon:"5"}
...
const PLACE_DAY_APPEARANCES = {
  ...
  "bryant-park":[{dayId:"2026-08-22",panel:"evening",focusKey:"arrival-evening-2"},{dayId:"2026-08-24",panel:"movie"},{dayId:"2026-08-30",panel:"walk",focusKey:"departure-walk-0"}],
  ...
  grandcentral:[{dayId:"2026-08-26",panel:"morning",focusKey:"morning-stop-2"},{dayId:"2026-08-30",panel:"walk",focusKey:"departure-walk-3"}]
};
...
const MATYLDA_ATLAS = [
  ...
  { icon:"≈", title:"Pier 35 · huśtawki nad rzeką", meta:"Dzień 7 · 28.08", text:"Krótki fotograficzny i wypoczynkowy finał po zakupach: huśtawki, zielona ściana i Manhattan Bridge.", dayId:"2026-08-28", panel:"pier35", placeId:"pier-35" },
