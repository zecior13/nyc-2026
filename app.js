...
function renderQueensGuide(day, guide) {
  ...
  <section class="day-panel" data-panel="route" hidden><div class="section-heading-row"><h3>Queens · od portu do kortów</h3><span>sprawdzono ${guide.checked}</span></div><p class="panel-intro">Numery 1–5 są identyczne na mapie i w opisie. Przejazd linią 7 jest łącznikiem, a nie osobnym miejscem.</p><div class="route-list">${guide.route.map((x,i)=>{const numbers=["1","2","→","3","4","5"];return `<article class="route-stop" data-content-key="q-stop-${i}"><div class="route-number ${i===2?"route-number-transit":""}">${numbers[i]}</div><div><span class="mini-kicker">${x.time}</span><h4>${x.title}</h4><p>${x.text}</p><div class="look-box"><strong>Rozejrzyj się:</strong> ${x.look}</div><button class="audio-chip" data-audio-key="qroute-${i}">▶ Posłuchaj na miejscu</button>${x.related?.length?`<div class="related-row">${x.related.map(r=>`<button data-related-panel="${r.panel||"stories"}" ${r.key?`data-related-key="${r.key}"`:""}>${r.label} ›</button>`).join("")}</div>`:""}</div></article>`}).join("")}</div></section>
...
const DAY_MAP_CONFIGS = {
  ...
  "2026-08-25": { title:"The Met → Central Park → Broadway", background:"53% 29% / 265% auto", points:[
    {panel:"overview",title:"Hotel",note:"Wyjazd na Upper East Side",x:24,y:82,icon:"H"},{panel:"museum",title:"The Met",note:"Wybrane obszary muzeum i pełny katalog dzieł",x:65,y:29,icon:"1"},{panel:"park",title:"Central Park",note:"Bethesda, Bow Bridge i Strawberry Fields",x:47,y:48,icon:"2"},{panel:"rest",title:"Hotel · powrót",note:"Obowiązkowy odpoczynek",x:27,y:78,icon:"H"},{panel:"theatre",title:"Broadway",note:"Stranger Things",x:38,y:70,icon:"3"}
  ],modes:["transit","walk","transit","walk"]},
  "2026-08-26": { title:"Midtown → Long Island City → Flushing", background:"70% 42% / 190% auto", points:[
    {panel:"overview",title:"Hotel",note:"Start po zachodniej stronie East River, w Midtown Manhattan",x:12,y:43,icon:"H"},{panel:"route",key:"q-stop-0",title:"Gantry Plaza",note:"Punkt 1 · nabrzeże Long Island City po stronie Queens",x:34,y:47,icon:"1"},{panel:"route",key:"q-stop-1",title:"Pepsi-Cola Sign",note:"Punkt 2 · krótki spacer na północ nabrzeżem",x:38,y:38,icon:"2"},{panel:"route",key:"q-stop-3",title:"Flushing",note:"Punkt 3 · jedzenie i migracyjne centrum Queens",x:72,y:43,icon:"3"},{panel:"route",key:"q-stop-4",title:"Unisphere",note:"Punkt 4 · dziedzictwo World’s Fair",x:79,y:64,icon:"4"},{panel:"tennis",title:"US Open",note:"Punkt 5 · Mixed Doubles i stadion",x:86,y:53,icon:"5"},{panel:"links",title:"Hotel · powrót",note:"Powrót metrem po meczu",x:16,y:48,icon:"H"}
  ],modes:["transit","walk","transit","transit","walk","transit"]},
...
function configuredAdventureMap(day) {
  ...
  return `<section class="day-adventure-map"><div class="day-map-heading"><div><span class="mini-kicker">Dzień na mapie</span><strong>${config.title}</strong><p>Czerwone odcinki oznaczają przejazdy lub promy; żółte — spacer. Mapę można powiększać dwoma palcami.</p></div><button type="button" data-day-map-direct="links">↗ Mapy i trasy</button></div><div class="day-map-board configured-day-map" data-gesture-map style="--day-bg-position:${position};--day-bg-size:${size}"><div class="day-map-canvas"><svg viewBox="0 0 100 100" aria-hidden="true">${paths}</svg>${config.points.map(point=>`<button type="button" class="day-map-stop" style="--left:${point.x}%;--top:${point.y}%" data-day-map-panel="${point.panel}" ${point.key?`data-day-map-key="${point.key}"`:""} data-day-map-title="${point.title}" data-day-map-note="${point.note}"><span>${point.icon}</span><small>${point.title}</small></button>`).join("")}</div></div><div class="day-map-detail" id="dayMapDetail"><strong>Wybierz punkt dnia</strong><p>Krótki opis pojawi się tutaj, zanim przejdziesz do pełnego modułu.</p></div></section>`;
}
...
function dayAdventureMap(day) {
  ...
  return `<section class="day-adventure-map village-adventure-map">
    <div class="day-map-heading"><div><span class="mini-kicker">Dzień na mapie</span><strong>Guggenheim → Village → Blue Note</strong><p>Czerwona linia oznacza przejazd; żółta — spacer. Mapę można powiększać dwoma palcami.</p></div><button type="button" data-day-map-direct="links">↗ Mapy i trasy</button></div>
    <div class="day-map-board" data-gesture-map><div class="day-map-canvas">
