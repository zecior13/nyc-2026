...
function renderQueensGuide(day, guide) {
  ...
  <section class="day-panel" data-panel="morning" hidden><div class="section-heading-row"><h3>Poranek · ikoniczny Midtown</h3><span>07:00–11:20</span></div><p class="panel-intro">Jedna logiczna trasa od hotelu przez Empire State i SUMMIT do katedry, a potem metro do Long Island City.</p><div class="route-list">${guide.morning.route.map((x,i)=>`<article class="route-stop"><div class="route-number">${i+1}</div><div><span class="mini-kicker">${x.time}</span><h4>${x.title}</h4><p>${x.text}</p><div class="look-box"><strong>Rozejrzyj się:</strong> ${x.look}</div></div></article>`).join("")}</div><article class="event-card"><h4>Praktycznie w SUMMIT</h4><ul>${guide.morning.practical.map(x=>`<li>${x}</li>`).join("")}</ul></article><button class="museum-day-link" type="button" data-open-place="summit">Otwórz miejsce: SUMMIT One Vanderbilt <span>›</span></button></section>
